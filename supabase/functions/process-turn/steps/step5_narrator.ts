...
10. ВЗАИМОДЕЙСТВИЕ МЕЖДУ ИГРОКАМИ (Player-to-Player Interaction):
   - В партии играют живые игроки! Когда действие направлено на другого игрока (разговор, обращение, вопрос, предложение путешествовать вместе, передача предмета, помощь):
     ...
     * В тексте для инициатора: опиши его слова, обращение или предложение к сопартийцу по имени.
     * В тексте для игрока-цели: красочно опиши, как сопартиец обращается именно к нему, окликает по имени или зовёт в путешествие. НЕ придумывай отказы или длинные реплики за живого игрока — он сам ответит в свой ход!
     * Если сформирован отряд (факт в knowledge) — художественно опиши готовность держаться плечом к плечу.
     * Если передаётся предмет или наносится урон — это отражено в knowledge и hp_status, опиши это художественно.
    ...
    try {
      ...
      const cleanNarrativeText = (raw: string): string => {
        ...
        return raw
          ...
          // Удаляем CJK иероглифы (японский, китайский, корейский)
          .replace(/[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff\uac00-\ud7af]/g, "")
          // Удаляем латинские слова ≥3 букв окружённые кириллическим контекстом или пробелами
          // Исключение: не трогаем одиночные буквы (e.g. "d20") и числа
          .replace(/([\u0400-\u04ff\s])[a-zA-Z]{3,}([\u0400-\u04ff\s,.])/g, "$1$2")
          // Дополнительно убираем английские ing-формы между русскими словами
          .replace(/\b[a-zA-Z]+ing\b/g, "")
          .replace(/\b[a-zA-Z]{5,}\b/g, "") // любые длинные англ. слова (5+)
          .replace(/[ \t]{2,}/g, " ")
          .replace(/\n{3,}/g, "\n\n")
          .trim();
      };
