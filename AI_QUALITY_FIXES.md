...
> Создан по итогам e2e тестов 03-09. Обновлён 15.09.2026 после полного анализа пар. **35/35 тестов ✅**
...
| 15 | `"возвращаюсь в таверну"` → `talk` (должно быть `move`) | ✅ Исправлено (промпт) | `231a453` |
| 16 | `"затачиваю палку → копьё"` → `harvest_ambient` (должно быть `craft_recipe`) | ✅ Исправлено (промпт) | `231a453` |
| 17 | CJK+Кириллица микс (`遠кий`) — фильтр оставлял кириллический хвост | ✅ Исправлено (токен целиком) | `231a453` |
| 18 | `move` без нарратива при `actions_count=0` | ⚠️ Роутер не создаёт action для свободного движения | — |
| 19 | Энкаунтер триггернулся но NPC не спаунился | ✅ Исправлено (spawn handler) | `b1c3b84` |
| 20 | `encounter_triggered` не читался в `index.ts` | ✅ Исправлено (spawn handler) | `b1c3b84` |
| 21 | `targeted` поиск врага не делал бросок кубика | ✅ Исправлено (3x multiplier + roll) | `b1c3b84` |
| 22 | `danger_level` локации не влиял на шанс встречи | ✅ Исправлено (safe=2%, normal=12%, danger=35%, lethal=65%) | `b1c3b84` |
| 23 | Порог энкаунтера 2.5% — слишком мал для любой зоны | ✅ Исправлено (динамическое масштабирование) | `b1c3b84` |
