...
| 15 | `"возвращаюсь в таверну"` → `talk` (должно быть `move`) | ✅ Исправлено (промпт) | `—` |
| 16 | `"затачиваю палку → копьё"` → `harvest_ambient` (должно быть `craft_recipe`) | ✅ Исправлено (промпт) | `—` |
| 17 | CJK+Кириллица микс (`遠кий`) — фильтр оставлял кириллический хвост | ✅ Исправлено (смешанный токен удаляется) | `—` |
| 18 | `move`/`search` без нарратива — только системные факты | 🔍 Анализируется | — |
| 19 | Динамический энкаунтер (волк) — классификатор не спаунит врага | ⚠️ Ограничение архитектуры | — |
