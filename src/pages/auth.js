...
export function renderAuth(container) {
    ...
    container.innerHTML = `
      <div class="page page-centered auth-page">
        <div class="auth-container" style="max-width: 480px;">
          ...
          <div id="googleAuthSection" style="${dbConfig.isCustom ? 'display: none;' : ''}">
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (dbConfig.isCustom) {
        toast.warning(
          'В вашей пользовательской БД вход через Google не включён (требуется Client ID из Google Cloud). Используйте форму входа ниже по Email и Паролю — она работает сразу!',
          9000
        );
      } else {
        toast.error(errMsg || 'Ошибка Google-авторизации (код 400)');
      }
      btn.disabled = false;
